...
export function validateCertConfig(config) {
  ...
  const sslCert =
    typeof config.sslCert === 'string' ? config.sslCert.trim() : '';
...
const validateFilePaths = (resolvedCertConfig) => {
  ...
  for (const [key, certPath] of Object.entries(resolvedCertConfig)) {
    ...
    if (!['.pem', '.crt'].includes(path.extname(sanitizedPath))) {
      errors.push(
        `Invalid file format for "${key}". Only .pem or .crt files are allowed.`
      );
    }
    ...
    try {
      ...
      errors.push(
        `The certificate file at "${sanitizedPath}" is not readable.`
      );
    }
  }
