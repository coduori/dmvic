const validateSecretsConfig = (
  ...
  const providedKeys = Object.keys(configData).map((key) =>
  ...
  for (const [key, allowedValues] of Object.entries(requiredValues)) {
    if (key in configData && !allowedValues.includes(configData[key])) {
      errors.push(
        `Invalid value: ${configData[key]} for secret: "${key}". Allowed values: ${allowedValues.join(', ')}.`
      );
    }
  }
