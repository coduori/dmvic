...
const certificateIssuanceSchema = object({
    ...
    vehicleChassisNumber: string().strict().trim().min(4).max(15).required(),
    vehicleMake: string().strict().min(1).max(50).optional(),
    vehicleModel: string().strict().min(1).max(50).optional(),
    vehicleValue: number()
        .min(1)
        .when('coverType', {
