...
const certificateIssuanceSchema = object({
    ...
    .strict()
        ...
        .max(15)
        .test(
            'has-aphabets-dashes-integers-only',
            'vehicleEngineNumber can only contain aphabets, dashes and integers',
            (value) => {
    ...
    vehicleChassisNumber: string()
        ...
        .test(
            'has-aphabets-dashes-integers-only',
            'chassisNumber can only contain aphabets, dashes and integers',
            (value) => {
                if (!/^[A-Za-z0-9-]+$/.test(value)) return false;
                if (value.startsWith('-') || value.endsWith('-')) return false;
                return !value.includes('--');
    ...
    recipientPhoneNumber: number()
        ...
        .required(),
    vehicleBodyType: string().strict().trim().max(50).min(1).optional(),
