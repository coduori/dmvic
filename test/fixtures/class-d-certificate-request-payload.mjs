...
const basePayload = getBaseRequestPayload({
  motorClass: MOTOR_CLASS_OPTIONS.CLASS_D,
});

const getClassDCertificateRequestPayload = (overrides = {}) => {
  if (!('certificateType' in overrides)) {
    overrides.certificateType = cryptoPickOne(
      Object.keys(CLASS_D_CERTIFICATE_TYPE_OPTIONS)
    );
  }
  ...
  if (
    Object.keys(CLASS_D_CERTIFICATE_TYPE_OPTIONS_COMMERCIAL).includes(
      selectedCertificateType
    )
  ) {
