...
describe('initialize DMVIC Configurations', () => {
    ...
    it('should throw an error for invalid initialization configurations', async () => {
        ...
        await expect(initialize(undefined)).rejects.toThrow('Invalid configuration. Expected an object.');
        await expect(initialize('invalid')).rejects.toThrow('Invalid configuration. Expected an object.');
        await expect(initialize({})).rejects.toThrow('Configuration must include "secrets" and "certificates".');
    });
